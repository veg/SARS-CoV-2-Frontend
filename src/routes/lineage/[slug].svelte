  ...
  onMount(async () => {
    ...
    if (taskRes.status === 200) {
      ...
      R.forEach(d => {
          ...
          d.hyphyVisionUrl = "http://vision.hyphy.org/" + method + "?resultsUrl=" + encodeURIComponent(d.resultsUrl);
          d.gene = gene;
          d.method = method;
      }, taskData);
    ...
    <div class="grid gap-4 grid-cols-3">
      {#each R.sortBy(R.prop('method'))(Object.values(groupedTaskData[group])) as task}
        <div class="grid grid-rows-2 grid-flow-col place-items-center { getStateBGColor(task) }">
          <div class="col-span-2 bg-white text-center w-full h-full p-2">{ task.method }</div>
          {#if task.state == "success"}
